...
`auto` 在外部 Key 已配置时使用的三模型路径。混合外部 OpenAI-compatible 模型 API 和 Codex 子智能体。
...
外部模型调用可能会把项目材料发送到本机之外。在 `auto` 或 `hybrid` 模式下，Skill 必须先说明会使用的供应商、模型和有边界的审查材料，再调用已配置的外部 reviewer。需要纯本地审查时使用 `builtin`。
